...
export class HomeComponent extends Unsubscribable {
  ...
  displayUserLocation() {
    if (this.map && this._lastKnownPosition) {
      const gPos = new google.maps.LatLng(this._lastKnownPosition.coords.latitude, this._lastKnownPosition.coords.longitude);
      if (this._marker) {
  ...
  getBeachDistance(beachId: number): string {
    const distanceMeter = this._locationModel.getBeachDistance(beachId);
    if (distanceMeter === 0) {
      return '';
    }
