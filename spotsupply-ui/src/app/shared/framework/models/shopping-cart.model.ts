...
export class ShoppingCartModel extends Model {
  ...
  constructor(private _store: Store<any>,
    ...
    super();
    const cart$ = this._store.select('cart');
  ...
  createShoppingCart(beachId: number) {
    if ([DataStatus.UNAVAILABLE, DataStatus.UNKNOWN].includes(this._cartAvailable)) {
      ...
      const shoppingCart: ShoppingCart = new ShoppingCart(undefined, beachId);
  ...
  addProduct(product: Product) {
    const cartItem: CartItem = new CartItem(-1, null, product.id);
