  ...
  connect(): Observable<UserData[]> {
    ...
    return Observable.merge(...displayDataChanges)
      .map(() => {
        ...
        const filteredData = sortedData.slice().filter((item: UserData) => {
          let searchStr = (item.name + item.color).toLowerCase();
          return searchStr.indexOf(this.filter.toLowerCase()) !== -1;
        });
        const pagedData = filteredData.splice(this._paginator.pageIndex * this._paginator.pageSize, this._paginator.pageSize);
        return pagedData;
      });
  }
