...
public interface ShoppingCartRepository extends JpaRepository<ShoppingCart, Long> {
    ...
    @Transactional
    @Query(value = "select cart FROM ShoppingCart cart WHERE sessionId = :sessionId")
    ShoppingCart findBySessionIdWithLock(@Param("sessionId") String sessionId);

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Transactional
    @Query(value = "select cart FROM ShoppingCart cart WHERE user = :user AND status IN (:cartStatuses)")
    ShoppingCart findByUserAndStatusInWithLock(@Param("user") User user, @Param("cartStatuses") List<CartStatus> cartStatus);
